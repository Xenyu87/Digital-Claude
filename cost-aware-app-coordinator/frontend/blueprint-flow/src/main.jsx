...
function FlowBoard({ root }) {
  ...
  const [viewMode, setViewMode] = useState('flows');
  ...
  const boardHome = useMemo(() => {
    const allVisible = visibleNodes.filter((node) => !node.hidden);
    const brokenFlows = (payload.flows || []).filter((item) => item.status === 'rotto');
    const partialFlows = (payload.flows || []).filter((item) => item.status === 'parziale');
    const completeFlows = (payload.flows || []).filter((item) => item.status === 'completo');
    const brokenNodes = allVisible.filter((node) => node.data.auditStatus === 'rotto');
    const weakNodes = allVisible.filter((node) => node.data.auditStatus === 'debole');
    const confirmedNodes = allVisible.filter((node) => node.data.auditStatus === 'certo');
    const firstFromIds = (ids = []) => ids.map((id) => nodes.find((node) => node.id === id)).find(Boolean);
    return [
      {
        key: 'broken',
        label: 'Flussi rotti',
        count: brokenFlows.length || brokenNodes.length,
        text: brokenFlows[0]?.next_step || brokenNodes[0]?.data?.auditFix || 'Nessun blocco evidente nella vista attuale.',
        target: brokenFlows[0] ? firstFromIds(brokenFlows[0].node_ids) : brokenNodes[0],
      },
      {
        key: 'check',
        label: 'Da verificare',
        count: partialFlows.length || weakNodes.length,
        text: partialFlows[0]?.next_step || weakNodes[0]?.data?.auditReason || 'Nessun punto debole evidente nella vista attuale.',
        target: partialFlows[0] ? firstFromIds(partialFlows[0].node_ids) : weakNodes[0],
      },
      {
        key: 'ok',
        label: 'Confermati',
        count: completeFlows.length || confirmedNodes.length,
        text: completeFlows[0]?.chain || confirmedNodes[0]?.data?.summary || 'Nessun flusso confermato nella vista attuale.',
        target: completeFlows[0] ? firstFromIds(completeFlows[0].node_ids) : confirmedNodes[0],
      },
    ];
  }, [payload.flows, visibleNodes, nodes]);
