...
- 2026-06-14 — drain.py `create_lessons_from_failed_tasks` filtro frasi conversazionali — nuovo `_CONFIRMATION_RE` scarta task partial il cui `summary` è solo una risposta conversazionale ("ok", "procedi", "confermato"...) prima di creare una lezione — risolve ~25 lezioni "ineffective" rumorose su `miglioramento_skill` trovate da session-analyst
- 2026-06-14 — SKILL.md §15 anti-loop bug rescue — 3 regole leggere: "State cause before fixing" (hypothesis esplicita prima di ogni fix non banale), "Same-hypothesis guard" (blocca 2° tentativo che ripete la stessa hypothesis non falsificata), "Error delta check" (confronta errore prima/dopo per capire se il fix ha mancato il bersaglio) — design da agente architect (opus), motivate dall'incidente submi/xmanager (fix in loop con Sonnet)
- 2026-06-14 — SKILL.md §15 escalation modello main agent — dopo 2 fix consecutivi falliti sullo stesso bug, stop e suggerimento esplicito `/model opus` per il main agent invece di riprovare in loop con lo stesso modello — segnalato dall'utente: su submi/xmanager Sonnet ha dato fix errati in loop, mai suggerito Opus, poi risolto subito passando manualmente a Opus 4.8
- 2026-06-15 — SKILL.md §3 + skill-sync.md + maintenance-compaction.md — novità CLI giu 2026: flag `--fallback-model <a,b>` (≥2.1.166) per overload/indisponibilità modello — NON chiave settings.json, NON copre loop di risposte sbagliate; `/reload-skills` (≥2.1.152) post sync_skill.py senza riavvio; `--safe-mode` (≥2.1.169) e Stop-hook block cap (≥2.1.147, `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`) per troubleshooting skill/hook/plugin
- 2026-06-15 — SKILL.md §9 — nota nested subagents (Claude Code ≥2.1.172, depth ≤5, ~7×/livello) — default depth 1, usa solo per pattern già previsti (mar-reviewer)
- 2026-06-13 — coordination_log.py `_detect_category` fix falsi negativi skill_improvement — `_SKILL_RE` allargata a "check/controlla/verifica ... skill"; nuovo `_touches_skill_files()` forza categoria "miglioramento_skill" se la sessione legge/modifica file sotto `.claude/skills/digital-claude/` indipendentemente dal testo del primo prompt — risolve 60 sessioni miscategorizzate come "modifica" (audit/fix della skill dal progetto dashboard)
...
- 2026-05-21 — references progressive-loading/specialist-agents/maintenance-compaction — /plugin projected context cost come segnale budget; esempio flag dispatched + nota wshobson/agents (no adozione); reminder revisione baseline modelli ad ogni minor CLI — da CLI v2.1.143 + research plan-1
...
- 2026-05-13 — scripts/validate_skill.py — esteso REQUIRED_SECTIONS a sezioni portanti + supporto glob asset citati — protegge da rimozioni/regressioni in refactor futuri
...
- 2026-05-02 — SKILL.md — aggiunte sezioni "Selezione del modello" (smallest capable model), "Working loop" (ciclo budget→contesto→piano→implementazione→verifica→chiusura), "Definition of Done" (conferma user-facing per UI a rischio medio/alto)
